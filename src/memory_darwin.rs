#![cfg(target_os = "macos")]
use mach::{kern_return::{KERN_SUCCESS, KERN_PROTECTION_FAILURE}, message::mach_msg_type_number_t, port::{MACH_PORT_NULL, mach_port_name_t, mach_port_t}, 
vm::{mach_vm_protect, mach_vm_read_overwrite, mach_vm_write, mach_vm_allocate, mach_vm_deallocate}, 
...
pub fn write_memory(target_task: u32, _address: usize, buffer: &Vec<u8>) -> Result<(), u32> {
    ...
    match proc_protect(target_task, region_address, region_size, VM_PROT_READ | VM_PROT_WRITE | VM_PROT_EXECUTE) {
        Ok(_) => (),
        Err(e) => {
    ...
    //
    proc_protect(target_task, region_address, region_size, region_protection as _).unwrap();
...
//
fn get_protection(target_task: u32, _address: mach_vm_address_t) -> Result<(u32, usize, usize), u32> {
